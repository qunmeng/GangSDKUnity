...
#import "CodoneChatTopLoadMoreView.h"
#import "GangTools.h"
#import "GangUIFont.h"

@implementation CodoneChatTopLoadMoreView
...
-(void)hasChangeToState:(CodoneRefreshState)state{
    ...
    switch (state) {
        case State_CanPullOrClick:
            self.label.text = [GangTools getLocalizationOfKey:@"点击或下拉加载"];
            self.indicator.hidden = YES;
            [self.indicator stopAnimating];
            break;
        case State_CanPull:
            self.label.text = [GangTools getLocalizationOfKey:@"下拉加载"];
            self.indicator.hidden = YES;
            [self.indicator stopAnimating];
            break;
