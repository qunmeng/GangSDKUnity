//
//  GangEditDeclarationViewController.m
//  GangSDK
//
...
@interface GangEditDeclarationViewController ()
...
@implementation GangEditDeclarationViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.label_title.text = [GangTools getLocalizationOfKey:@"编辑宣言"];
    self.label_title.textColor = [UIColor colorFromHexRGB:GangColor_pop_editDreamWord_title];
    [self.tv setTextColor:[UIColor colorFromHexRGB:GangColor_pop_editDreamWord_inputMessage]];
    [self.tv setTintColor:[UIColor colorFromHexRGB:GangColor_pop_editDreamWord_inputMessage]];
    ...
    [self.label_holderText setTextColor:[UIColor colorFromHexRGB:GangColor_pop_editDreamWord_placeHolder]];
...
- (IBAction)btn_edit_click:(id)sender {
    NSString *str_decalaration = self.tv.text;
    if (str_decalaration.length==0) {
        if ([self.tv isFirstResponder]) {
            [self.view toastTheMsg:@"请输入宣言内容!" atPosition:POSITION_TOP duration:1.5];
        }else{
            [self gang_toast:@"请输入宣言内容!"];
        }
        return;
    }else if ([CodoneTools countTheStrLength:str_decalaration]>30){
        if ([self.tv isFirstResponder]) {
            [self.view toastTheMsg:@"宣言最多30字!" atPosition:POSITION_TOP duration:1.5];
        }else{
            [self gang_toast:@"宣言最多30字!"];
        }
        return;
    }
    ...
    [GangSDKInstance.groupManager updateGangDecalaration:str_decalaration success:^(id  _Nullable data) {
